namespace Assets.MyAssets.Scripts.Match
{
    ...
    public sealed class TurnStateMachine
    {
        ...
        public GameState WinPlayer { get; private set; }
        public int TurnCount { get; private set; }

        /// <summary>지금이 플레이어의 턴인가(시작 전·종료 후가 아닌가).</summary>
        public bool IsPlayerTurn => Current == GameState.Player1 || Current == GameState.Player2;

        /// <summary>현재 턴 플레이어의 playerList 인덱스.</summary>
        public int CurrentPlayerIndex => Current == GameState.Player1 ? PLAYER_1_INDEX : PLAYER_2_INDEX;

        public void Reset()
        {
            Current = GameState.None;
            WinPlayer = GameState.None;
            TurnCount = 0;
        }

        public void BeginFirstTurn()
        {
            Current = GameState.Player1;
        }

        public void ChangeTurn()
        {
            Current = Current == GameState.Player1 ? GameState.Player2 : GameState.Player1;
        }
        ...
        /// <summary>
        /// 승자를 확정하고 종료 상태로 넘어간다. 무승부일 때는 GameState.None을 넘긴다.
        /// </summary>
        public void FinishWith(GameState winner)
        {
            WinPlayer = winner;
        ...
        {
            return maxTurn == TurnCount && Current == GameState.Player2;
        }
    }
}
